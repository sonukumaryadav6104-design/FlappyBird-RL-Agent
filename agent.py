...
class Agent:
    def __init__(self , param_set):
        ...
        self.epsilon_deccay = params["epsilon_deccay"]  
    ...
    def run(self , is_training = True , render =False):
        ...
        if is_training:
            ...
            # copy the wt & vals from policy => target
        ...
        for episode in itertools.count():
            ...
            if is_training:
            # epsilon deccay
                epsilon = max(epsilon * self.epsilon_deccay , self.epsilon_min)
           ...
           # env.close() : becauser we are going to stop it manually 

    def optimize(self, mini_batch, policy_dqn, target_dqn):
    # mini_batch = list of (state, action, next_state, reward, terminated)
...
if __name__ == "__main__":
    ...
    parser.add_argument("hyperparameters", help="Hyperparameter set to use e.g. flappy1")
    parser.add_argument("--train", help="Training mode", action="store_true")
    args = parser.parse_args()

    dql = Agent(param_set=args.hyperparameters)   # ✅ now works
